...
#
# Also REVERTED same-day: a brief detour built scripts/coverage-audit.js
# as a Meta-API EU-only cross-check. That was the wrong tool — SC's
# company/ads endpoint already returns ALL countries with no filter (no
# country param = global), so we already fetch US+EU+everywhere on the
# main path. The Meta API's EU-only commercial-ad restriction made the
# audit a noisy false-positive generator. Ripped before merging into a
# user-facing flow.
